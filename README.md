...
| | 흔한 접근 | 이 프로젝트 |
|---|---|---|
| 논문 재현 | 라이브러리 모델 fine-tuning, "비슷하게 나옴" | 처음부터 구현 → **공식 체크포인트를 로드해 출력이 같음(같은 기계 비트 단위, 기계 간 1e-6)**을 테스트로 고정 → 5분할×3시드 재현표 → 논문 서술과 공식 코드가 다른 3곳 ablation |
...
**우리 학습**은 처음부터 구현한 코드로 CPU 에서 250 epoch 학습한 값(학습 시드 0)이다. 학습 시드 3개(0·1·2)로 재면 ADE 표본 표준편차가
hotel·zara2 는 0.01, eth·univ·zara1 은 0.04–0.05 이고
3시드 평균의 5분할 평균 ADE 는 0.47 다 — 논문(0.44)과의 차이는 이 시드 분산 안이며, 시드별 표는
