...
**평가 결과가 말하는 것** (README §4.5, `results/collision_eval.json`, `results/collision_eval_dsafe2.json`): d_safe 1.0 m 에서는
현재 거리만 쓰는 지오펜스가 AP·AUROC 모두 가장 좋고(AUROC 0.914 vs 학습 모델 0.903, AP 0.084 vs 0.026) — 4.8 s 예측 오차가 d_safe 와
같은 크기이기 때문이다. d_safe 2.0 m 에서는 학습 모델이 AP 0.465 vs 0.341, 최고 F1 0.508 vs 0.383 으로 앞서고, 비슷한 오경보율에서
재현율과 선행시간이 모두 높다. 결정적(μ) 변형은 두 설정 모두 AUROC 0.66–0.72 로 무너지므로 위험은 반드시 샘플 분포로 계산해야 한다.
운영 구성은 "지오펜스 반경(넓게) 안의 쌍을 risk 로 우선순위화" — 소비자의 `AlertPolicy` 는 그 위에서 연속 프레임·쿨다운으로 알림 수를 줄인다.
